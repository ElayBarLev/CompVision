...
## Bonus: on-device phone demo (NOT a project requirement)
> This is an **extra "flex", not part of the brief's deliverables.** The required edge story
> is the measured params/size/latency in `src/utils/edge_metrics.py`; this just *physically*
> runs the trained detector on a **Pixel 6a, in the phone's browser** (onnxruntime-web, fully
> on-device — no app, no server). Full write-up: [`docs/08_edge_phone_deploy.md`](docs/08_edge_phone_deploy.md).

**Files this bonus adds:**
| File | Purpose |
|---|---|
| `src/edge/export_onnx.py` | Export a `weights/*_best.pt` checkpoint to ONNX (lowers `min_size` for phone speed) and self-verify it with onnxruntime. |
| `web/index.html`, `web/app.js` | The in-browser live-camera demo (onnxruntime-web, WASM). |
| `web/model.onnx` | The exported model the page loads (generated by the export script). |
| `docs/08_edge_phone_deploy.md` | Why browser-not-native, the `min_size`/WASM choices, demo steps, fallbacks. |

**Run it:**
```bash
python src/edge/export_onnx.py --weights weights/fasterrcnn_mobilenet_aug_best.pt  # -> web/model.onnx (+verify)
python -m http.server -d web 8000        # laptop dry-run at http://localhost:8000
# then publish web/ via GitHub Pages (HTTPS, needed for the camera) and open the URL on the phone
```
